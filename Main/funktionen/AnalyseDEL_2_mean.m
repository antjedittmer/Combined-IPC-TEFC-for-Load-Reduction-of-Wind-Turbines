function [AnalyseDELout] = AnalyseDEL_2_mean(SimRunFolderPath,BaselineCase,Channelgruppen,BaselineCaseFolderPath,DoOptimization)
             ...
             for i = 1 : length(Channelgruppen)
                 if Channelgruppen(i).MemberTest == "Passed"
                     if Channelgruppen(i).NumTest == "Passed"
                         ...
                         DELMean = mean(cell2mat(DELDataGruppe(i).DELs),1);
             ...
             for i = 1 : length(Channelgruppen)
                 if Channelgruppen(i).MemberTest == "Passed"
                     if Channelgruppen(i).NumTest == "Passed"
                         % Mittelwerte der DELs einer Gruppe Berechnen
                         DELMean = mean(cell2mat(DELDataGruppe(i).DELs),1);
                         DELMeanBaseline = mean(cell2mat(DELDataGruppeBaseline(i).DELs),1);

                         % Relative Aenderung des Mittelwerts im Vergleich zum
                         % Baselinecase berechnen                    
                         rel_delta_mean = 100*(DELMean - DELMeanBaseline) ./ DELMeanBaseline; 
