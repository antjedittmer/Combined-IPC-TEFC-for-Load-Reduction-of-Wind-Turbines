function [AnalyseStatsout] = AnalyseStats_4(SimRunFolderPath, BaselineCase, BaselineCaseFolderPath, DoOptimization, StatsChannel)
 ...
 for z = LoopStart : length(folderNames)                                           % loop über Windtestfälle    
     switch DoOptimization
         case "false"
             ...
             for w = 1 : length(StatsChannel)
                 % Nach Zeile mit gewähltem Channel suchen 
                 strarray = convertCharsToStrings(table2array(Data.sheet_1(:,1)));            
                 ExcelZeile = strmatch(StatsChannel(w),strarray,"exact");
                 if isempty(ExcelZeile)
                     fprintf('\nDer angegebene Channelname %s ist in der Analyse_Stats.xls-Datei nicht enthalten.\n',StatsChannel(w));
                     AnalyseStatsout = [];
                     return;
                 end
                 ...
                 results.(sprintf('%s',StatsChannel(w))).RelDeltaMean   = rel_delta_mean;
                 results.(sprintf('%s',StatsChannel(w))).RelDeltaStdDev = rel_delta_StdDev;
                 results.(sprintf('%s',StatsChannel(w))).sheets         = sheets(2:end);
                ...
                fprintf('Relative Änderungen des Mittelwerts und der Standardabweichung von %s im Vergleich zum Referenzfall %s:\n',StatsChannel(w), BaselineCase);
                fprintf('------------------------------------');
         ...
         case "true"
             ...
             for w = 1 : length(StatsChannel)
                 ...
                 results.(sprintf('%s',StatsChannel(w))).RelDeltaMean   = rel_delta_mean;
                 results.(sprintf('%s',StatsChannel(w))).RelDeltaStdDev = rel_delta_StdDev;
                 results.(sprintf('%s',StatsChannel(w))).sheets         = sheets(1:end);
