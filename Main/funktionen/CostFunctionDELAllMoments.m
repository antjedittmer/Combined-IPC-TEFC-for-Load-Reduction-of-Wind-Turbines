function [CostsOut] = CostFunctionDELAllMoments(AnalysePSDout,AnalyseStatsout,AnalyseDELout,Testcase)
...
for i = 1 : length(fieldnames(AnalyseStatsout))                            % Loop ueber Windtestfaelle
    ...
    for k = 1 : height(AnalyseDELout.(sprintf('%s',Fieldnames1(i))))
        
        DELresults = AnalyseDELout.(sprintf('%s',Fieldnames1(i))).results(k,1).RelDeltaMax(TestcaseIndex);
        ...
        CostsM1(k) = DELresults;  % DELs aller Momente
    
    end
    
    CostsM2 = CostsM1(1:3);
    Costs(i) = max(CostsM2) / 100;
